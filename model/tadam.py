...
def get_arguments():
    ...
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'eval', 'train_classifier'])
    # Dataset parameters
    ...
    parser.add_argument('--number_of_steps', type=int, default=int(30000), help="Number of training steps")
...
def build_feature_extractor_graph(images, flags, num_filters, beta=None, gamma=None, is_training=False,
    ...
    if is_training and is_64way is False:
        ...
        h = tf.reshape(h, shape=(1, embedding_shape[0], -1), name='reshape_to_separate_tasks_generic_features')
    return h
...
def build_task_encoder(embeddings, labels, flags, is_training, reuse=None, scope='class_encoder'):
    ...
    with conv2d_arg_scope:
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if is_training:
                task_encoding = tf.reshape(task_encoding, shape=(flags.num_tasks_per_batch, flags.num_classes_train,
...
def build_cosine_head(features_generic, task_encoding, flags, is_training, scope='cosine_head'):
    ...
    with tf.variable_scope(scope):
        ...
        cosine_mult = tf.Variable(flags.metric_multiplier_init,
                                  trainable=(is_training and flags.metric_multiplier_trainable), name='cosine_mult',
                                  dtype=tf.float32)
...
def get_film_gamma_beta_net(h, i, j, num_filters, flags, is_training, reuse):
    ...
    for l in range(flags.film_num_layers):
        ...
        beta = slim.fully_connected(beta, num_outputs=num_filters, activation_fn=None, normalizer_fn=None, reuse=reuse,
                                    weights_initializer=ScaledVarianceRandomNormal(factor=0.1),
                                    weights_regularizer=tf.contrib.layers.l2_regularizer(scale=flags.weight_decay),
                                    scope='projection_beta' + str(i) + str(j) + str(l), trainable=is_training)
        gamma = slim.fully_connected(gamma, num_outputs=num_filters, activation_fn=None, normalizer_fn=None, reuse=reuse,
                                     weights_initializer=ScaledVarianceRandomNormal(factor=0.1),
                                     weights_regularizer=tf.contrib.layers.l2_regularizer(scale=flags.weight_decay),
...
def get_film_params(features_task_encode, num_filters_list, flags, reuse=False, is_training=False,
                    scope='film_params_raw'):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i, num_filters in enumerate(num_filters_list):
            for j in range(flags.num_units_in_block):
                beta, gamma = get_film_gamma_beta_net(h, i, j, num_filters=num_filters, flags=flags,
...
def build_feat_extract_pretrain_graph(images, flags, is_training):
    ...
    with tf.variable_scope('Model'):
        feature_extractor_encoding_scope = 'feature_extractor_encoder'
        features = build_feature_extractor_graph(images=images, flags=flags, is_training=is_training,
                                                 num_filters=num_filters, scope=feature_extractor_encoding_scope,
        ...
        # Classification loss
        logits = slim.fully_connected(features, flags.num_classes_pretrain, activation_fn=None, normalizer_fn=None,
                                      reuse=False, scope='pretrain_logits', trainable=is_training)
    return logits
...
def train(flags):
    ...
    with tf.Graph().as_default():
        ...
        supervisor = tf.train.Supervisor(logdir=log_dir, init_feed_dict=None, summary_op=None,
                                         init_op=tf.global_variables_initializer(), summary_writer=summary_writer,
